 ...
 */
metadata {
	definition (name: "MyQ Lock Door", namespace: "brbeaird", author: "Jason Mok/Brian Beaird/Barry Burke", vid: "generic-contact-4", mnmn: "SmartThings") {
...
def close() { 
	...
	if (currentState && allowedStates.contains(currentState)) {
		...
		runIn(30, refresh, [overwrite: true]) 	// Force a sync with Contact/3D Sensor after 30 seconds (accomodate the ~10 second "waiting" period)
	} else {
