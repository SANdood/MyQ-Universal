 ...
 */
metadata {
	definition (name: "MyQ Garage Door Opener-NoSensor", namespace: "brbeaird", author: "Jason Mok/Brian Beaird/Barry Burke", vid: "generic-contact-4", ocfdevicetype: "oic.d.garagedoor", mnmn: "SmartThings") {
...
def push() {	
	...
	switch(doorState) {
		case 'open':		// obviously
		case 'opening':		// try to abort the open
			close()
			break;
		case 'closed':		// obviously
		case 'closing':		// try to abort the close
		case 'waiting':		// try to abort the pending close
		case 'stopped':		// try not to hurt anyone
			open()
			break;
	}
}
*/
def open()  { 
	def allowedStates = ['closed','closing','waiting','stopped']
	def currentState = device.latestValue('door')
	if (currentState && allowedStates.contains(currentState)) {
		...
		runIn(20, resetToUnknown, [overwrite: true])	//Force a sync withSensors after 20 seconds
	} else {
		log.debug "Ignoring open() request because door is currently ${currentState} (${allowedStates.toString()[1..-2]})"
	}
}
def close() { 
	def allowedStates = ['open','opening']
	def currentState = device.latestValue('door')
	if (currentState && allowedStates.contains(currentState)) {
		log.debug "Garage door sending 'close' command."
		parent.notify("${device.displayName} sending 'close' command.")
		parent.sendCommand(getMyQDeviceId(), "close")
		// updateDeviceStatus("closing")		// Now handled in the parent (in case we have an Acceleration sensor, we can display "waiting" state)
		runIn(30, resetToUnknown, [overwrite: true]) 	// Force a sync with Contact/3D Sensor after 30 seconds (accomodate the ~10 second "waiting" period)
	} else {
		log.debug "Ignoring close() request because door is currently ${currentState} (${allowedStates.toString()[1..-2]})"
	}
}	
...
def getMyQDeviceId(){	    
    if (device.currentState("myQDeviceId")?.value)
        ...
        def newId = device.deviceNetworkId.split("\\|").last()       
        sendEvent(name: "myQDeviceId", value: newId, display: false , displayed: false)
        return newId
    }	
}
...
def updateDeviceAcceleration(activity) {
